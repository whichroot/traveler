"""Check owned pinned staging, asynchronous copies, and prefix retirement."""
...
with tempfile.TemporaryDirectory() as directory:
    temp = Path(directory)
    ir = temp / 'staging.ll'; package = temp / 'staging.tvcp'
    ...
    for name in ('gate', 'faults'):
        run(TVC, HERE / f'cuda_staging_{name}.tv', '--emit', 'obj', '-llc', LLC, '-o', temp / f'{name}.o')
    ...
    if CUDA:
        ...
        print(f'CUDA staging PASS: native SM{SM}, pinned slots and event-prefix retirement')
